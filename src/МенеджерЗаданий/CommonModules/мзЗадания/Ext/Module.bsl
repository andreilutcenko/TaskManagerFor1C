...
Процедура ЗапуститьНовыхИсполнителей(Знач АктивныеИсполнители)
	...
	РежимПланировщика = ПолучитьАлгоритмПланировщика();
	Если РежимПланировщика = Перечисления.мзАлгоритмПланировщика.ОдноЗаданиеНаОбработчик Тогда
		ЗапуститьНовыхИсполнителейПоОдномуНаЗадание(АктивныеИсполнители);
	Иначе
		ЗапуститьНовыхИсполнителейВРежимеАвтораспределения(АктивныеИсполнители);
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗапуститьНовыхИсполнителейПоОдномуНаЗадание(Знач АктивныеИсполнители)
	КоличествоЗаданийВОчереди = ПолучитьКоличествоЗаданийВОчереди();
	ОграничениеПоКоличествуИсполнителей = ПолучитьОграничениеНаКоличествоИсполнителей();
	КоличествоДобавляемыхИсполнителей = Мин(ОграничениеПоКоличествуИсполнителей - АктивныеИсполнители.Количество(), КоличествоЗаданийВОчереди);
...
Процедура ЗапуститьНовыхИсполнителейВРежимеАвтораспределения(Знач АктивныеИсполнители)
	КоличествоЗаданийВОчереди = ПолучитьКоличествоЗаданийВОчереди();
	ОграничениеПоКоличествуИсполнителей = ПолучитьОграничениеНаКоличествоИсполнителей();
	КоличествоДобавляемыхИсполнителей = Мин(ОграничениеПоКоличествуИсполнителей - АктивныеИсполнители.Количество(), КоличествоЗаданийВОчереди);
	Если КоличествоДобавляемыхИсполнителей > 0 Тогда
		...
		Для каждого КлючЗадания Из Задания Цикл
			КлючИдентификацииИсполнителя = СгенерироватьКлючИдентификацииИсполнителя();
			ЗапуститьИсполнителяСАвтораспределением(КлючИдентификацииИсполнителя);
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьАлгоритмПланировщика() Экспорт
	
	ЗначениеНастройки = Константы.мзАлгоритмПланировщика.Получить();
	Возврат ?(ЗначениеЗаполнено(ЗначениеНастройки), ЗначениеНастройки, Перечисления.мзАлгоритмПланировщика.ОдноЗаданиеНаОбработчик);
	
КонецФункции

Процедура УстановитьАлгоритмПланировщика(Знач АлгоритмПланировщика) Экспорт
	Константы.мзАлгоритмПланировщика.Установить(АлгоритмПланировщика);
КонецПроцедуры
...
Процедура ЗапуститьИсполнителяСАвтораспределением(Знач КлючИдентификацииИсполнителя)
	Параметры = Новый Массив;
	Параметры.Добавить(КлючИдентификацииИсполнителя);
	ФоновыеЗадания.Выполнить("мзЗадания.Исполнитель_РабочийПотокАвтораспределения", Параметры, КлючИдентификацииИсполнителя);
КонецПроцедуры
...
Процедура Исполнитель_РабочийПотокАвтораспределения(Знач КлючИдентификацииИсполнителя) Экспорт
	
	Пока Истина Цикл
		
		РазмерПорции = 100;
		Задания = ПолучитьЗаданияИзОчереди(РазмерПорции);
		...
		Для Каждого КлючЗадания Из Задания Цикл
			...
			НачатьТранзакцию();
			Попытка
				ЗаблокироватьЗаданиеДляИзменения(КлючЗадания);
			Исключение
				// таймаут на блокировке
				ОтменитьТранзакцию();
				ИмяСобытия = Метаданные.Подсистемы.МенеджерЗаданий.Синоним + ".Примечания";
				ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Примечание,,,
					НСтр("ru = 'Не удалось дождаться блокировки задания: " + КлючЗадания + ".'"));
				Продолжить;
			КонецПопытки;
			...
			Если ДеталиЗадания <> Неопределено Тогда
				Прервать;
			КонецЕсли;
		...
		Если ДеталиЗадания = Неопределено Тогда
			// ни одно задание из порции взять в работу не удалось
			ИмяСобытия = Метаданные.Подсистемы.МенеджерЗаданий.Синоним + ".Примечания";
			ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Примечание,,,
				НСтр("ru = 'Нет заданий для взятия в работу. Завершаю поток обработчика.'"));
			Возврат;
		КонецЕсли;
		...
		Попытка
			...
			ОтметитьЗавершениеЗадания(КлючЗадания);
		Исключение
			ЗаписатьОшибкуВЖР("ОшибкаВыполненияМетода", КлючЗадания, ОписаниеОшибки());
			ПоставитьНаПаузу(КлючЗадания);
		КонецПопытки;
...
Процедура ЗаблокироватьЗаданиеДляИзменения(Знач КлючЗадания)
	
	Блокировка = Новый БлокировкаДанных;
	БлокЗаписи = Блокировка.Добавить("РегистрСведений.мзЗадания");
	БлокЗаписи.УстановитьЗначение("Ключ", КлючЗадания);
...
Функция ПолучитьКоличествоЗаданийВОчереди() Экспорт
...
Процедура ПоставитьНаПаузу(Знач КлючЗадания)
	
	Набор = ПолучитьЗаполненныйНаборЗаписейПоКлючуЗадания(КлючЗадания);
	Если Набор.Количество() = 1 Тогда
		Запись = Набор[0];
		Запись.Состояние = Перечисления.мзСостоянияЗаданий.НаПаузе;
		Набор.Записать();
	КонецЕсли;
...
Процедура ЗаписатьОшибкуВЖР(Знач Событие, Знач КлючЗадания, Знач ТекстОшибки)
	ИмяСобытия = Метаданные.Подсистемы.МенеджерЗаданий.Синоним + "." + Событие;
	Комментарий = "Задание = " + КлючЗадания + "
	|" + ТекстОшибки;
